...
export default function RemoveItemFromCart({ workout, fetchWorkout }) {
    ...
    const removeToggle = async () => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                const response = await fetch(`https://fitnessapp-api-ln8u.onrender.com/workouts/completeWorkoutStatus/${workout}`, {
                    method: 'PATCH',
                    headers: {
                ...
                if (data.error !== "Error in updating an Workout." || data.error !== "Workout not found" ) {
                    Swal.fire({
                        title: 'Success',
                        icon: 'success',
    ...
    return (
        <Button className="btnComplete me-2" variant="success" size="sm" onClick={removeToggle}>
            Complete
        </Button>
    )
}
